...
const DialogueBox = preload("res://addons/dialogue_manager/example_balloon/example_balloon.gd")
...
func dialogue_box(title: String, local_resource: DialogueResource = null, extra_game_states: Array = []) -> void:
	var dialogue_line = yield(get_next_dialogue_line(title, local_resource, extra_game_states), "completed")
	if dialogue_line != null:
		var balloon = preload("res://function/dialogue/Dialogue_V1.tscn").instance()
		balloon.dialogue_line = dialogue_line
		get_tree().current_scene.add_child(balloon)
		dialogue_box(yield(balloon, "actioned"), local_resource, extra_game_states)
